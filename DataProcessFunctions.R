...
Extract_province_data <- function(data){
  ...
  for (i in 1:31){
    ...
    tem_data_province = data[data$province==i,] #Extract province data
...
Happiness_na_inter <- function(data,need_inter_col){
  #To interpolate the data based on the happiness level
  ...
  for (i in 1:length(need_inter_col)){
    # Find the data which has NA value, then interpolate based on the happiness level
...
Interpolate_BMI_Combine_Leisure_trust <- function(data){
  ...
  #deletethe old features leisure_1 到 Leisure_11 
  data = data[,-which(names(data) %in% feature_leisure )]
  dim(data)
  
  #combine trust features
...
Combine_All_features <- function(data){
  ...
  dim(data)
  #delete property_1 to property_2 
  data = data[,-which(names(data) %in% property_own )]
  
  ## combine property_0_8
  ...
  #delete public_service_1 to public_service_9
  data = data[,-which(names(data) %in% feature_society )]
  tem_data_public_delete = data
  
  #combine Meida_1 Media_2 Media_3 
  ...
  #class - class_10_before is called new feature change
  ...
  #class_10_after - class :new feature attitude
  ...
  #new feature --> father and mother's average edu
...
Province_centra_scale <- function(data){
  ...
  #Special normalization for province model
